...
export const createTrip = (trip: Trip): Trip => {
  const newTrip: Trip = { ...trip, id: uuidv4(), createdAt: Date.now(), status: TripStatus.TRIP_STATUS_PENDING };
  ...
  if (newTrip.vehicleType !== undefined) { // Indicates a pooled trip request
    ...
    for (const pool of ridePools) {
      if (
        pool.status === PoolStatus.POOL_STATUS_OPEN &&
...
export const getOpenPools = (vehicleType?: VehicleType): RidePool[] => {
