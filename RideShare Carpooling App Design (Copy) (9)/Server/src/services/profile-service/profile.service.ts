...
// Define a local interface that matches the structure we want to work with in the service
...
export const createUserProfile = async (profile: UserProfile): Promise<UserProfile> => {
  const newProfile: UserProfile = { ...profile, id: profile.id || uuidv4(), uniqueId: Math.random().toString(36).substring(2, 10).toUpperCase(), friends: [] };
