...
export type UpdateAdvanceBookingRequest = Partial<CreateAdvanceBookingRequest>;
...
export type AdvanceDispatchOutcome =
  | 'DISPATCHED'          // 2+ confirmed, driver search started
  | 'WAIT_FOR_MATCH'      // 1 confirmed, still time to backfill
  | 'ASK_USER_TO_EXTEND'  // 1 confirmed, no runway left to backfill
  | 'CANCEL_POOL'         // 1 confirmed and nothing left to try
  | 'CANCELLED';          // 0 confirmed
