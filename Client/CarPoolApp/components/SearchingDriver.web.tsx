...
import { View, Text, Animated, TouchableOpacity } from 'react-native-web';
import { MapPin, Users, Clock, Plus, RefreshCw } from './Icons';
...
export default function SearchingDriver({ onCancel, onDriverFound, onSearchExpired }: SearchingDriverProps) {
  ...
  const handleSearchExpiry = useCallback(async () => {
    ...
    // Notify server to mark the pool as SEARCH_EXPIRED so others don't see it
    if (selectedPool?.id) {
      try {
        ...
        <View className="flex-1 items-center justify-center gap-6">
          ...
          <TouchableOpacity
            ...
            style={{
              ...
              opacity: isCreatingNewPool ? 0.7 : 1,
            }}
          >
            {isCreatingNewPool ? (
              <RefreshCw size={24} color="#ffffff" className="animate-spin" />
            ) : (
