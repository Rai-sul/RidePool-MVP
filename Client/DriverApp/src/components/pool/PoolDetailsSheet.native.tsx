      ...
      >
        ...
        >
          ...
          {!isExpanded ? (
                    ...
                    <View className="flex-row items-center gap-3 flex-1">
                      ...
                      <View className="flex-1">
                        <View className="flex-row items-center gap-2 mb-1">
                          <Text className="font-semibold text-lg" style={{ color: colors.text }}>{pool.customers.length} Passengers</Text>
                          {pool.isPriority && (
                            <Badge style={{ backgroundColor: colors.success }}>
                              <Text style={{ color: colors.primaryText }}>⭐ Priority</Text>
                            </Badge>
                          )}
                        </View>
                        <View className="flex-row items-center gap-2">
                          <Text className="text-sm" style={{ color: colors.textSecondary }}>{pool.distance}km</Text>
                          <Text className="text-sm" style={{ color: colors.textSecondary }}>•</Text>
                          <Text className="text-sm" style={{ color: colors.textSecondary }}>{pool.estimatedTime}min</Text>
                        </View>
                      </View>
                    </View>
                    <View className="flex-row items-center gap-2">
                      <Text className="text-3xl font-bold" style={{ color: colors.success }}>৳{pool.totalEarnings}</Text>
                      <Button
                        size="icon"
                        variant="ghost"
              ...
              <View className="p-4 flex-row items-center justify-between border-b" style={{ backgroundColor: colors.card, borderBottomColor: colors.border }}>
                <View>
                  ...
                  <Text className="text-sm mt-1" style={{ color: colors.textSecondary }}>
                    {pool.customers.length} passengers • {pool.distance}km
                  </Text>
                </View>
                ...
                <View className="p-5 gap-5">
                  ...
                  <View className="flex-row gap-3">
                    <View className="flex-1 p-4 rounded-xl border items-center" style={{ backgroundColor: isDark ? '#064E3B' : '#D1FAE5', borderColor: isDark ? '#059669' : '#A7F3D0' }}>
                      ...
                      <Text className="text-2xl font-bold mt-1" style={{ color: colors.success }}>৳{pool.totalEarnings}</Text>
                    ...
                    <View className="flex-1 p-4 rounded-xl border items-center" style={{ backgroundColor: isDark ? '#1E3A8A' : '#DBEAFE', borderColor: isDark ? '#2563EB' : '#93C5FD' }}>
                      ...
                      <Text className="text-2xl font-bold mt-1" style={{ color: '#2563EB' }}>{pool.distance}</Text>
                      <Text className="text-xs mt-1" style={{ color: colors.textSecondary }}>km</Text>
                    </View>
                    <View className="flex-1 p-4 rounded-xl border items-center" style={{ backgroundColor: isDark ? '#581C87' : '#F3E8FF', borderColor: isDark ? '#9333EA' : '#D8B4FE' }}>
                      ...
                      <Text className="text-2xl font-bold mt-1" style={{ color: '#9333EA' }}>{pool.estimatedTime}</Text>
                  ...
                  <View className="rounded-xl p-4" style={{ backgroundColor: colors.card, borderWidth: 1, borderColor: colors.border }}>
                    <View className="flex-row items-center justify-between mb-3">
                      ...
                      <Badge variant="outline" style={{ borderColor: colors.border }}>
                        <Text style={{ color: colors.text }}>{pool.customers.length * 2} stops</Text>
                      </Badge>
                    </View>
                    <View className="gap-2">
                      ...
                      <View className="flex-row items-center gap-2">
                        ...
                        <Text className="text-sm" style={{ color: colors.textSecondary }}>First Pickup: {pool.firstPickup}</Text>
                      </View>
                      <View className="flex-row items-center gap-2">
                        <View className="w-2 h-2 rounded-full" style={{ backgroundColor: colors.error }} />
                        <Text className="text-sm" style={{ color: colors.textSecondary }}>Final Drop: {pool.finalDestination}</Text>
                      </View>
                    </View>
                  </View>
                  ...
                  {/* Passengers */}
                  <View>
                    <Text className="font-semibold mb-3" style={{ color: colors.text }}>Passengers ({pool.customers.length})</Text>
                    <View className="gap-3">
                      {pool.customers.map((customer, index) => (
                        <View key={customer.id} className="rounded-xl p-4" style={{ backgroundColor: colors.card, borderWidth: 1, borderColor: colors.border }}>
                          <View className="flex-row items-start gap-3">
                            ...
                            <View className="flex-1">
                              <View className="flex-row items-center gap-2 mb-2">
                                <Text className="font-semibold" style={{ color: colors.text }}>{customer.name}</Text>
                                <Badge variant="outline" style={{ borderColor: colors.border }}>
                                  <Text className="text-xs" style={{ color: colors.text }}>⭐ {customer.rating}</Text>
                                </Badge>
                              </View>
                              <View className="gap-2">
                                <View className="flex-row items-start gap-2">
                                  <Text className="font-medium" style={{ color: colors.success }}>↑ Pickup:</Text>
                                  <Text className="flex-1" style={{ color: colors.text }}>{customer.pickup}</Text>
                                </View>
                                <View className="flex-row items-start gap-2">
                                  <Text className="font-medium" style={{ color: colors.error }}>↓ Drop:</Text>
                                  <Text className="flex-1" style={{ color: colors.text }}>{customer.destination}</Text>
                                </View>
                              </View>
                            </View>
                          </View>
                  ...
                  <View>
                    ...
                    <View className="rounded-xl p-4" style={{ backgroundColor: colors.card, borderWidth: 1, borderColor: colors.border }}>
                      <View className="gap-3">
                        ...
                        {pool.customers.map((customer, index) => (
                          <View key={`pickup-${customer.id}`} className="flex-row items-center gap-3">
                            ...
                            <Text className="text-sm font-medium flex-1" style={{ color: colors.success }}>
                              Pick up {customer.name.split(' ')[0]} at {customer.pickup}
                            </Text>
                          </View>
                        ))}
                        {pool.customers.map((customer, index) => (
                          <View key={`dropoff-${customer.id}`} className="flex-row items-center gap-3">
                            <View className="w-8 h-8 rounded-full items-center justify-center" style={{ backgroundColor: colors.error }}>
                              <Text className="font-semibold" style={{ color: colors.primaryText }}>{pool.customers.length + index + 1}</Text>
                            </View>
                            <Text className="text-sm font-medium flex-1" style={{ color: colors.error }}>
                              Drop {customer.name.split(' ')[0]} at {customer.destination}
                            </Text>
                          </View>
                        ))}
                      </View>
            ...
            {/* Accept Button - Absolutely positioned at bottom of container */}
            <View style={{ position: 'absolute', bottom: 0, left: 0, right: 0, paddingHorizontal: 20, paddingBottom: 20, backgroundColor: colors.background }}>
              ...
              >
                <Text className="text-lg font-semibold" style={{ color: colors.primaryText }}>Accept Pool - ৳{pool.totalEarnings}</Text>
              </Button>
            </View>
            </View>
          )}
