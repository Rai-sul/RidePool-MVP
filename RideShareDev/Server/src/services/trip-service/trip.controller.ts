...
import { Trip, RidePool, VehicleType } from './trip.service';
...
export const getOpenPools = (req: Request, res: Response) => {
  ...
  let vehicleTypeEnum: VehicleType | undefined;

  if (vehicleType === 'car') {
    vehicleTypeEnum = VehicleType.VEHICLE_TYPE_CAR;
  } else if (vehicleType === 'cng') {
    vehicleTypeEnum = VehicleType.VEHICLE_TYPE_CNG;
  }

  const openPools = tripService.getOpenPools(vehicleTypeEnum);
  res.json(openPools);
};
