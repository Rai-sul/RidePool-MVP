...
\graphicspath{{images/}} % Path to Image files (FIXED: must be {{folder/}})
...
% ---- Added for the RidePool chapters (tables and figures) ----
\usepackage{booktabs} % \toprule, \midrule, \bottomrule in tables
\usepackage{tabularx} % Tables that fit the text width
\usepackage{float}    % [H] placement: keep tables/figures where they are written
\usepackage{rotating} % sidewaysfigure for the wide EER and schema diagrams
\newcolumntype{L}{>{\raggedright\arraybackslash}X} % left-aligned wrapping column
\newcolumntype{C}{>{\centering\arraybackslash}X}   % centred wrapping column
% --------------------------------------------------------------
...
\usepackage[intoc]{nomencl} % The nomenclature package can be used to generate and format a nomenclature using MakeIndex. (FIXED: "intoc" adds it to the TOC on the correct page)
...
\addbibresource{bibliography/references.bib} % Imports bibliography file (Chapter 2 references)
\addbibresource{bibliography/ridepool.bib}   % References used in Chapters 1, 3, 4 and 5
...
\printnomenclature % TO  generation of the Nomenclature file (added to TOC by the "intoc" option)
\cleardoublepage
...
\printbibliography[heading=bibintoc] % Where the bibliography will be printed (FIXED: "bibintoc" adds it to the TOC on the correct page)

% ********************************** Appendices ********************************
% The two appendices below are the template's sample pages ("How to install LaTeX",
% "Overleaf: GitHub for LaTeX projects"). They are switched off because they are not
% part of the RidePool report. Remove the % signs to bring them back.
%
%\newpage
%\phantomsection
